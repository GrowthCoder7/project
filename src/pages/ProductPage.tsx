...
export default function ProductDetails() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-8 py-10 pt-28">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-5">
            ...
            <div className="flex gap-2">
              {product.sizes.map((size) => (
                <button
                  ...
                  onClick={() => setSelectedSize(size)}
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
                    selectedSize === size ? "bg-amber-700 text-white" : "bg-gray-200 text-gray-800"
                  }`}
                >
                  {size}
                </button>
          ...
          <div className="border-t pt-4">
            <button onClick={toggleDescription} className="flex justify-between w-full text-lg font-semibold text-gray-800">
              Description
              {isDescriptionOpen ? <ChevronUp /> : <ChevronDown />}
            </button>
            {isDescriptionOpen && <p className="mt-2 text-gray-600">{product.description}</p>}
          </div>

          {/* ℹ️ Additional Information Toggle */}
          <div className="border-t pt-4">
            <button onClick={toggleInfo} className="flex justify-between w-full text-lg font-semibold text-gray-800">
              Additional Information
              {isAdditionalInfoOpen ? <ChevronUp /> : <ChevronDown />}
            </button>
            {isAdditionalInfoOpen && (
          ...
          <div className="flex gap-4 mt-6">
            <button
              ...
              className="px-6 py-3 bg-amber-700 text-white rounded-lg shadow-md hover:bg-amber-800 transition-all"
            >
              Add to Cart
            </button>
            <button
              onClick={handleAddToWishlist}
              className="px-6 py-3 border border-amber-700 text-amber-700 rounded-lg hover:bg-amber-700 hover:text-white transition-all"
            >
              Add to Wishlist
            </button>
          </div>
